...
const signup = async (req: Request, res: Response) => {
  ...
  try {
    if (await buscaUsuarioPorEmail(usuario.email))
      return res.status(400).json({ msg: "Já existe usuário com o email informado." });
    const newUsuario = await createUsuario({...usuario, tipoUsuarioId: TiposUsuarios.CLIENT});
...
const login = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).json({msg: "Usuário autenticado com sucesso", isAdmin});
  } catch (e) {
