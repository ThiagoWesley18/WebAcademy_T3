...
import { getUsuarios, buscaUsuarioPorId, buscaUsuarioPorEmail, createUsuario, updateUsuario, deleteUsuario } from "./usuario.service";
  ...
  */
  ...
  try {
    if (await buscaUsuarioPorEmail(usuario.email))
      return res.status(400).json({ msg: "Já existe um usuário com o email informado." });
